...
const { fetchSoccerEventData } = require('./soccerevent.service');
const SoccerUnsettled = require('../models/soccerUnsettled.model');
const exchanges = require('../config/exchanges');

const fetchUnsettledSoccerBets = async () => {
  try {
    const results = await Promise.allSettled(
      exchanges.map((ex) =>
        axios.get(`${ex.baseUrl}/api/bets/settlement/unsettled-bets?sport=soccer`, {
          headers: {
            'X-Api-Key': ex.apiKey,
          },
          timeout: 15000,
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled') {
        const rows = result.value.data?.data?.soccer || [];
        rows.forEach((row) => {
          all.push({
            ...row,
            exchangeKey: ex.key,
        ...
        console.error(`Soccer unsettled API error (${ex.key}):`, status || result.reason?.message);
      }
    });
    ...
    if (error.code === 'ECONNABORTED') {
      console.error('Soccer unsettled API error: Request timeout - API took longer than 15 seconds');
    } else if (error.response) {
      console.error('Soccer unsettled API error: Server responded with status', error.response.status);
    } else if (error.request) {
      console.error('Soccer unsettled API error: No response received from server');
    } else {
      console.error('Soccer unsettled API error:', error.message);
    }
    throw error;
  }
};
...
const enrichUnsettledWithSections = async () => {
  const unsettled = await fetchUnsettledSoccerBets();
  ...
  await Promise.all(
    eventIds.map(async (eventId) => {
      try {
        ...
        console.error(`Failed to fetch soccer event data for eventId ${eventId}:`, error.message);
  ...
  try {
    if (bulkOps.length > 0) {
      await SoccerUnsettled.bulkWrite(bulkOps, { ordered: false });
    }
  } catch (dbError) {
    console.error('Failed to persist enriched soccer unsettled data:', dbError.message);
  }
