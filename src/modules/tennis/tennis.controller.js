...
const TennisSettlement = require('../../models/tennisSettlement.model');
const TennisEventState = require('../../models/tennisEventState.model');
...
const getUnsettledSummary = async (req, res, next) => {
  try {
    ...
    const [result] = await TennisUnsettled.aggregate(pipeline);
...
const getUnsettledByEventId = async (req, res, next) => {
  try {
    ...
    const settlements = await TennisSettlement.find({ eventId: String(eventId) }).lean();
...
const settleMatchOdds = async (req, res, next) => {
  try {
    ...
    if (resolvedWinnerName == null || resolvedWinnerName === '') {
      const row = await TennisUnsettled.findOne({
        eventId: String(eventId),
    ...
    await TennisSettlement.findOneAndUpdate(
...
// POST /api/tennis/unsettled/events/set-open
// Body: { eventId, openEvent: true | false }
const setTennisEventOpen = async (req, res, next) => {
  try {
    ...
    const state = await TennisEventState.findOneAndUpdate(
      { eventId: String(eventId) },
